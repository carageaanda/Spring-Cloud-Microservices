...
public class CurrencyExchangeController {
    ...
    public EntityModel<String> fallbackRetrieveExchangeValue(String from, String to, Throwable exception) {
        logger.error("Fallback method called due to an error: {}", exception.getMessage());

        String errorMessage = "Service temporarily unavailable. Please try again later.";

        WebMvcLinkBuilder allExchangesLinkBuilder = WebMvcLinkBuilder
                .linkTo(WebMvcLinkBuilder.methodOn(this.getClass())
                        .getAllCurrencyExchanges());

        EntityModel<String> entityModel = EntityModel.of(errorMessage);
        entityModel.add(allExchangesLinkBuilder.withRel("allCurrencyExchanges"));
